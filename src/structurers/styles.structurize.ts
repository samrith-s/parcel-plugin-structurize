    ...
    return new Promise(resolve => {
        ...
        move(Path.join(dist, match), Path.join(dist, folder))
            .then(async () => {
                await markups.forEach(async document => {
                    ...
                    await allStyles.forEach(async style => {
                        ...
                        try {
                            let content: any = await fs.readFileSync(stylePath);
                            content = content.toString().replace(oldFilePath, style.href);
            ...
            .catch(e => {
                ...
                console.log(chalk.red.bold('Error while structurizing styles. Please check structures.'));
                // eslint-disable-next-line no-console
