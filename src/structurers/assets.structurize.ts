...
import rewriteCSSUrls from 'css-url-rewrite';
        ...
        move(Path.join(dist, match), Path.join(dist, folder))
            .then(() => {
                markups.forEach(async document => {
                    const allAssets = await document.querySelectorAll(`img, source, link[rel*="icon"]`);
                    const allStyles = await document.querySelectorAll('style, link[rel="stylesheet"]');
                    ...
                    await allStyles.forEach(async style => {
                        if (style.tagName === 'STYLE') {
                            style.textContent = rewriteUrls(style.textContent, path);
                            return style.textContent;
                        }
                        if (isRemote(style.href, origin + prefix)) return;

                        const filePath = Path.join(dist, Path.basename(style.href));
            ...
            .catch(e => {
                ...
                console.log(chalk.red.bold('Error while structurizing assets. Please check structures.'));
                // eslint-disable-next-line no-console
